...
# render rather than after the search. The check is skipped while the dimension
# is unset, because the labelling sections do not run on that pass and a search
# render should not need a key.
...
**Admissibility** comes first, because it is a yes or no. A residual variance is the part of an item the factors do not explain, and it cannot be negative. When one comes out negative the solution is impossible rather than merely poor, and the usual cause is asking for more factors than the battery supports: one of them splits off and absorbs more than all of an item's variance. Such a solution is out regardless of how good its fit indices look, and the fit indices will look good, because an inadmissible solution is one that has fitted the sample too closely. Read `min_resid` beside the flag rather than trusting the flag alone. The search fits each rotated solution through `cfa()` with an EFA block, which leaves residual variances unbounded, so a Heywood case appears here as a negative number; `lavaan`'s `efa()` bounds them at zero instead and shows the same failure as a residual sitting suspiciously close to it. The block form is used because `efa()` treats every column of the data frame it is given as an indicator, and the sampling weight column that design-weighted estimation requires is one of those columns.
...
pa <- future_map(1:cfg$n_pa, function(i) {
  ...
  do.call(what = rbind)
# seed = TRUE, not NULL: this is the one loop in either report that draws random
# numbers inside the workers rather than being handed them, and under seed = NULL
# the draws depend on the worker count, so the threshold moves between a
# sequential and a parallel render. seed = TRUE takes L'Ecuyer streams from the
# seed set above, which reproduces under any plan.
...
eig_tbl |>
  ...
  lca_table(align = "r",
            caption = "Design-weighted eigenvalues against a random-data
...
theta_cfa <- function(w_rep) {
  ...
  if (inherits(f, "try-error") || !lavInspect(f, "converged"))
    return(set_names(rep(NA_real_, length(par_names)), par_names))
  coef(f)[par_names]
}
...
param_tbl |>
  ...
  scale_fill_manual(values = c(`FALSE` = viridisLite::viridis(1, begin = 0.55),
                               `TRUE` = "grey75"),
                    labels = c(`FALSE` = "estimated", `TRUE` = "anchor,
                               fixed at 1")) +
...
#| echo: false
#| fig-height: 6
#| fig-cap: "The fitted measurement model. Item edges carry the standardised loading; the arc between factors carries their correlation."
...
# The factors are correlated, and a diagram that draws items but not the link
# between the factors hides the parameter that says whether two were warranted.
# An empty frame rather than NULL, so a one-factor solution draws nothing here
# instead of erroring.
fcor <- unclass(lavInspect(fit_w, "std")$psi)

node_pairs <- if (length(fac) > 1) {
  as_tibble(t(combn(seq_along(fac), 2)),
            .name_repair = function(x) c("i", "j")) |>
    mutate(y1 = nodes$ny[i], y2 = nodes$ny[j],
           rho = map2_dbl(i, j, function(a, b) fcor[a, b]))
} else {
  tibble(i = integer(), j = integer(), y1 = double(), y2 = double(),
         rho = double())
}
...
ggplot(edges) +
  geom_curve(aes(x = 0.20, y = ny, xend = 0.58, yend = y,
                 ...
                 colour = factor),
             curvature = 0.10) +
  geom_curve(data = node_pairs,
             aes(x = 0.20, y = y1, xend = 0.20, yend = y2),
             curvature = -0.55, linewidth = 0.4, colour = "grey40",
             inherit.aes = FALSE) +
  geom_label(data = node_pairs,
             aes(x = 0.135, y = (y1 + y2) / 2, label = sprintf("%.2f", rho)),
             size = 2.4, linewidth = 0, fill = "white",
             label.padding = unit(1.5, "pt"), inherit.aes = FALSE) +
  ...
  scale_x_continuous(limits = c(0.03, 1.02)) +
...
modindices(fit_w, sort = TRUE, minimum.value = 10) |>
  ...
  lca_table(align = "llrr", widths = c("12em", "12em", "6em", "6em"),
            caption = "Ten largest modification indices for residual
            covariances. A ranking, not a test.")
```
...
Two models carry it, as in the class report. Each factor is named in its own call by the **worker**, seeing only its own loadings, so one factor's name cannot be reasoned into place against another's. That isolation has a blind spot when a questionnaire section yields two related factors, which is exactly when two drafts come back as near-synonyms, so the same mechanical collision check runs afterwards and, only if it fires, one **editor** call edits the labels that overlap and nothing else. `cfg$llm_model_worker` and `cfg$llm_model_editor` name the two; an empty editor entry runs both roles on the worker. The domain readings in @sec-reading are the editor's for the same reason: they need every level of a demographic in view at once.
...
if (file.exists(lab_path)) {
  ...
  factor_labels <- map(fnames, function(fn) {
    obj <- parse_json_block(
      llm_chat(cfg, persona_cfa, role = "worker")$chat(
  ...
  # The same collision gate the class report uses, on the same instrument. Each
  # factor was named without seeing the other, so two factors drawn from one
  # questionnaire section can come back as near-synonyms with nothing in the
  # drafting step able to notice. The gate is mechanical and the editing call
  # happens only when it fires; descriptions are never touched.
  collided <- harmonise_due(factor_labels$Label, cfg)
  w <- llm_spec(cfg, "worker")
  e <- llm_spec(cfg, "editor")

  factor_labels <- factor_labels |>
    mutate(Label_draft = Label) |>
    harmonize_labels(cfg, unit = "FACTOR", persona = persona_cfa) |>
...
# Every use of factor_labels below indexes it by position in fnames, so the
# frozen file has to be in that order. A file the analyst re-sorted while
# editing a name, or one written under a different factor ordering, would
# otherwise attach each name to the wrong factor and pass the row-count check
# while doing it -- a presentation error with no symptom, which is the failure
# this freeze file exists to prevent. Match on the factor column instead.
missing_f <- setdiff(fnames, factor_labels$factor)
...
The tables below report each mean factor position with an unweighted and a design-based standard error side by side, and the summary line printed with them gives the median ratio of the two for this file. Two features of this survey hold that ratio down: the released weights are nearly constant, so weighting moves almost nothing, and a mean factor position within a demographic level averages over most of the sampling points, which caps how much the clustering can inflate its variance. Neither feature is a property of the method, so the ratio is read off the printed line on every file rather than carried over from this one.
...
## What the extreme scores are doing {#sec-tails}

The Bartlett column is wider than the design-based one by construction, and that much is the trade @sec-scoring describes. How much wider is not a constant, and that is the thing to check rather than assume. Under shrinkage alone the ratio of the two standard errors would be one number, roughly the reciprocal of the factor's reliability, in every cell of every demographic. A ratio that varies cell by cell is a different mechanism: a few respondents with very large scores, and cells that differ in how many of them they contain.

Where those respondents come from is not mysterious. For ordinal indicators the maximum likelihood position of a respondent who answered at one end of nearly every item has no bounded optimum, so the estimate runs out to the tail rather than settling. The table below counts how many items each group answered at the floor or the ceiling of its observed range; if the extreme scorers sit far above everyone else on that count, the tail is a boundary artefact rather than a set of genuinely extreme respondents.

The sensitivity is reported the same way a design effect is: as a ratio and a shift in units of the estimate's own standard error. Nothing is trimmed from the analysis. A respondent who answered at the floor of every item is a real respondent and the estimand includes them; the point of removing them here is to find out how much of the Bartlett column they account for, which is a question about the estimator rather than about the population.
...
What the pair demonstrates is the choice itself. This battery is `r length(cfg$items)` items of one format asking one kind of question about different objects, and its first eigenvalue dwarfs the second: one continuum, which a factor model represents in a fraction of the parameters a class model would need. The class report's battery is of mixed format spanning unrelated domains, where a household can own a computer and still have run short of food, and no single continuum holds that. Reading the two search sections against each other is the argument for when each model applies, and it is made by the data rather than asserted.
...
The structure search and the confirmatory fit use the same data. Where `cfg$cfa_factors` was set from this report's own exploratory results, the confirmatory fit is a re-expression of that structure rather than an independent test of it, and its fit indices are optimistic. The replicate intervals on the eigenvalues in @sec-search are the guard against reading noise as structure, and they are not a substitute for an independent sample. The two-factor solution fitted here departs from the single factor the eigenvalues support, on the substantive grounds stated in @sec-chosen.

The replicate count equals the number of primary sampling units, so joint tests across more parameters than there are replicates are unavailable. Individual standard errors and pairwise covariances are unaffected, which is all the pairwise contrasts in @sec-reading require.
