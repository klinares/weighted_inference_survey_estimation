...
# The excluded cases are idiosyncratic patterns rather than the tails of the
# factor: respondents at the floor on every item score without trouble.
...
# near the response boundary: the median absolute score is under 0.5 while the
# largest exceeds 18, so SD alone describes two dozen respondents rather than the
# spread of the factor. Excluding them moves no domain estimate by as much as its
# standard error on this file, which is the check the limitations section reports.
