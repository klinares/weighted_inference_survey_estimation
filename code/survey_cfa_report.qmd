...
lrt_tbl |>
  ...
  lca_table(align = "lrrrr",
            col.names = c("Comparison", "Chi-square", "df", "p naive", "p 
                          adjusted"),
            widths = fit_widths(5, first = 1.6), font_size = 8,
            caption = paste0("Nested comparison of successive factor solutions, 
                             naive and design-adjusted (deff ",
                             round(gdeff, 2),
                             "), a first-order approximation to a Rao-Scott 
                             correction. Read it as a guide, not an exact test.")
            )
```
...
glue("Weighting moves the factor means by a median of ",
     ...
     "{round(median(wide_gap$`se_Design-based` / 
     wide_gap$se_Unweighted), 2)} ",
...
cat("Wrote survey_with_factor.sav, three CSVs, and cfa_fit.rds to", 
    cfg$cfa_dir, "\n")
```
